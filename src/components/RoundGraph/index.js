...
const RoundGraph = (props) => {
  ...
  const drawRound = () => {    
    ...
    const setColor = total => {return total <= 50 ? colors.red : total >= 70 ? colors.green : colors.orange};
    const setFontSize = radius => {return radius <= 120 ? 11 : radius >= 180 ? 14 : 12};
    let radius = Math.min(width, height) / 1.9, spacing = 0.09;
    
    d3.select(roundRef.current).selectAll("*").remove();
    ...
    let field = d3.select(roundRef.current)
      .selectAll('g')
    ...
    d3.select(roundRef.current)
      ...
      .on('click', function(){        
        ...
        d3.select(roundRef.current).select('.text-all' + index).attr('opacity', 0);        
      })
  ...
  const drawLine = () => {
    ...
    let field = d3.select(lineRef.current)
      .append('g')
